...
def _make_silero_core():  # noqa: ANN202
    ...
    class Silero16kInferenceCore(torch.nn.Module):
        """Riva-compatible 576-sample recurrent inference graph."""
...
def build_engine(args: argparse.Namespace) -> None:  # noqa: C901, PLR0915
    ...
    # Silero carries its recurrent state across every 32 ms frame.  TensorRT
    # enables TF32 by default on Ampere and newer GPUs; the small per-frame
    # error then accumulates over a long recording and can move probabilities
    # across the VAD threshold.  Keep this engine in true FP32.  Throughput
    # comes from batching independent recordings, not reduced precision.
